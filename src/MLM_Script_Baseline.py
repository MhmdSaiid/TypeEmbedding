...
concept_vector = concept_vector_d['svd_vec']
    ...
    for l in tqdm(data_df_len.index):
        ...
        for batch in test_dataloader:
            ...
            with torch.no_grad():
                ...
                stacked_logits = torch.stack([x[ind] for ind,x in zip(bb_minds,o.logits)])
                stacked_logits_logsoftmax=F.log_softmax(stacked_logits,dim=-1)
                WE_module =  model.bert.embeddings.word_embeddings
                WE_matrix = WE_module.weight.detach()
                sim_scores = torch.nn.functional.cosine_similarity(WE_matrix,concept_vector.reshape(1,-1)).cpu().numpy().tolist()
                d = dict(zip(vocab,list(sim_scores)))
                sim_scores = torch.tensor(list(d.values())).to(device)
                stacked_logits_logsoftmax = stacked_logits_logsoftmax+ k *sim_scores



                bb_ans = [vocab[torch.argmax(x)] for x in stacked_logits_logsoftmax]

                top100s = [reversed(torch.argsort(x)[-101:]) for x in stacked_logits_logsoftmax]
...
if concept_vector is not None and k>0:
    with torch.no_grad():
